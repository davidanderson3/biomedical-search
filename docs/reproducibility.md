...
licensed files, links corpus text to CUIs, builds concept documents/vectors,
builds provenance and relationship indexes, runs paragraph evaluation plus
precision audit, writes a reproducibility manifest, and writes the server command
to `build/public/server_command.txt`.
...
python3 scripts/evidence_vectors.py link-corpus \
  --corpus build/public/pubmed_topics_corpus.jsonl build/public/europepmc_topics_corpus.jsonl build/public/pmc_oa_topics_corpus.jsonl \
  --label-index build/umls_biomedicine_search_label_index.sqlite \
  --out build/public/public_corpus_evidence.jsonl

python3 scripts/evidence_vectors.py build-docs-sqlite \
  --evidence build/public/public_corpus_evidence.jsonl \
